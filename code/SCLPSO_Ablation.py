# -*- coding: utf-8 -*-
# SCLPSO progressive variant experiment on CEC2017.
...
class ExperimentConfig:
    ...
    OUT_ROOT: str = "/public/home/sszzkli/.local/SCLPSO_REVISED/CEC2017_SCLPSO_PROGRESSIVE_VARIANTS_WITH_F2_PUBLICATION_PLOTS"
    ...
    CONV_AXIS_LABEL_SIZE: float = 9.0
    CONV_TICK_SIZE: float = 8.0
    CONV_PANEL_TITLE_SIZE: float = 11.0
    CONV_LEGEND_SIZE: float = 11.5


    RANK_AXIS_LABEL_SIZE: float = 12.0
    RANK_TICK_SIZE: float = 10.0
    RANK_TITLE_SIZE: float = 14.0
    RANK_VALUE_SIZE: float = 10.0


    BOXPLOT_FIG_WIDTH: float = 16.0
    BOXPLOT_FIG_HEIGHT: float = 11.0
    BOXPLOT_AXIS_LABEL_SIZE: float = 42.0
    BOXPLOT_TICK_SIZE: float = 36.0
    BOXPLOT_TITLE_SIZE: float = 42.0
    BOXPLOT_LINE_WIDTH: float = 3.0
...
def format_function_list(funcs: List[int]) -> str:
    if funcs == list(range(1, 31)):
        return "F1-F30 (30 functions)"
    if funcs == [1] + list(range(3, 31)):
        return "F1 and F3-F30 (29 functions; legacy setting)"
    return str(funcs)
...
def sclpso_nostate_revised(
    ...
    if bool(params.get("DISABLE_HC_INIT", False)):
        particles = uniform_init(pop_size_eff, dim, bounds, rng)
    else:
    ...
    while evaluator.fes < max_fes:
        ...
        if not bool(params.get("DISABLE_CL_BACKBONE", False)):
            for ii in range(n):
        ...
        if evaluator.remaining() > 0 and n >= 4 and not bool(params.get("DISABLE_SHADE", False)):
            if state == "exploration":
            ...
            p_num = max(2, int(round(float(params["SHDL_P"]) * n)))
            p_num = min(p_num, n)
        ...
        if evaluator.remaining() > 0 and not bool(params.get("DISABLE_DVA", False)):
            if state == "exploration":
        ...
        if evaluator.remaining() > 0 and not bool(params.get("DISABLE_ESCAPE", False)) and (state in ("stagnation", "escape")):
        ...
        if bool(params.get("DISABLE_LPSR", False)):
            target_n = particles.shape[0]
        else:
        ...
        recorder.update(evaluator.fes, best_fit, diversity=diversity, state=f"SCLPSO-{state}")
        iteration += 1
...
def lshade_algorithm(dim, func_num, pop_size, max_fes, bounds, so_path, seed) -> Dict[str, Any]:
    ...
    r_ninit = float(L_SHDL_PARAMS["R_NINIT"])
    r_arc = float(L_SHDL_PARAMS["R_ARC"])
    p_best_rate = float(L_SHDL_PARAMS["P"])
    h_size = int(L_SHDL_PARAMS["H"])
    n_min = int(L_SHDL_PARAMS["N_MIN"])
    ...
    while evaluator.fes < max_fes and n_current >= n_min:
        ...
        recorder.update(evaluator.fes, best, diversity=population_diversity(pop, bounds), state="L_SHDL")
...
def _run_sclpso_progressive_variant(dim, func_num, pop_size, max_fes, bounds, so_path, seed, changes: Dict[str, Any]) -> Dict[str, Any]:


    params = SCLPSO_PARAMS.copy()
    params["DISABLE_STATE_GUIDANCE"] = True
    params["DISABLE_DVA"] = True
    params["DISABLE_ESCAPE"] = True
    params.update(changes)
    return sclpso_nostate_revised(
        dim=dim,
        func_num=func_num,
        pop_size=pop_size,
        max_fes=max_fes,
...
def pso_algorithm(dim, func_num, pop_size, max_fes, bounds, so_path, seed) -> Dict[str, Any]:
    ...
    return _run_sclpso_progressive_variant(
        dim, func_num, pop_size, max_fes, bounds, so_path, seed,
        {"DISABLE_HC_INIT": True, "DISABLE_CL_BACKBONE": True,
         "DISABLE_SHADE": True, "DISABLE_LPSR": True}
    )


def hpso_algorithm(dim, func_num, pop_size, max_fes, bounds, so_path, seed) -> Dict[str, Any]:

    return _run_sclpso_progressive_variant(
        dim, func_num, pop_size, max_fes, bounds, so_path, seed,
        {"DISABLE_CL_BACKBONE": True, "DISABLE_SHADE": True, "DISABLE_LPSR": True}
    )


def clhpso_algorithm(dim, func_num, pop_size, max_fes, bounds, so_path, seed) -> Dict[str, Any]:
...
def shclpso_algorithm(dim, func_num, pop_size, max_fes, bounds, so_path, seed) -> Dict[str, Any]:
...
def sclpso_final_algorithm(dim, func_num, pop_size, max_fes, bounds, so_path, seed) -> Dict[str, Any]:
...
ALGORITHM_REGISTRY = {
    "PSO": pso_algorithm,
    "HPSO": hpso_algorithm,
    "CLHPSO": clhpso_algorithm,
    "SHCLPSO": shclpso_algorithm,
    "SCLPSO": sclpso_final_algorithm,
}
...
def win_tie_loss_against_acpso(summary_df: pd.DataFrame, target: str = "SCLPSO") -> pd.DataFrame:
    rows = []
    for dim, df_dim in summary_df.groupby("dimension"):
        algos = sorted(df_dim["algorithm"].unique())
        funcs = sorted(df_dim["function"].unique())
...
def plot_all_function_boxplots(raw_df: pd.DataFrame, out_root: str, final: bool = True) -> List[str]:
    ...
    box_label_size = int(getattr(CFG, "BOXPLOT_AXIS_LABEL_SIZE", 42))
    box_tick_size = int(getattr(CFG, "BOXPLOT_TICK_SIZE", 36))
    box_title_size = int(getattr(CFG, "BOXPLOT_TITLE_SIZE", 42))
    box_lw = float(getattr(CFG, "BOXPLOT_LINE_WIDTH", 3.0))
    fig_w = float(getattr(CFG, "BOXPLOT_FIG_WIDTH", 16.0))
    fig_h = float(getattr(CFG, "BOXPLOT_FIG_HEIGHT", 11.0))
    ...
    for (dim, func_num), df_case in ok.groupby(["dimension", "function"]):
        ...
        bp = ax.boxplot(
            ...
            whiskerprops={"linewidth": box_lw},
            capprops={"linewidth": box_lw},
            medianprops={"color": "black", "linewidth": box_lw + 0.6},
            meanprops={"color": "red", "linewidth": box_lw + 0.3},
            flierprops={"marker": "o", "markersize": 6, "markeredgewidth": 1.2, "alpha": 0.75},
        )
        ...
        for element in ["whiskers", "caps", "medians", "means"]:
            if element in bp:
                for item in bp[element]:
                    try:
                        item.set_linewidth(box_lw + (0.6 if element == "medians" else 0.0))
                    except Exception:
                        pass

        if use_log_scale:
            ax.set_yscale("log")
            ax.set_ylabel("Objective Value (log scale)", fontproperties=prop, fontweight="bold", fontsize=box_label_size)
            ax.yaxis.set_major_formatter(BoldMathFormatter())
            ax.yaxis.set_major_locator(LogLocator(base=10.0, numticks=10))
        else:
            ax.set_ylabel("Objective Value", fontproperties=prop, fontweight="bold", fontsize=box_label_size)

        ax.set_title(
            f"CEC2017 {int(dim)}D F{int(func_num):02d}",
            fontproperties=prop,
        ...
        ax.grid(True, axis="y", alpha=0.5, linewidth=1.5)
        ax.tick_params(axis="both", which="major", labelsize=box_tick_size, width=2.8, length=8)
        ...
        for label in ax.get_xticklabels() + ax.get_yticklabels():
            label.set_fontproperties(prop)
            label.set_fontsize(box_tick_size)
            label.set_fontweight("bold")
        ...
        fig.savefig(path, dpi=700, bbox_inches="tight", pad_inches=0.08)
        plt.close(fig)
        paths.append(path)
    return paths
...
SCLPSO_VERSION_DEFINITIONS = [
    {"algorithm": "PSO", "HCI": 0, "CL": 0, "SHDL": 0, "LPSR": 0,
     "description": "Basic PSO backbone."},
    {"algorithm": "HPSO", "HCI": 1, "CL": 0, "SHDL": 0, "LPSR": 0,
     "description": "PSO with Halton-chaotic initialization."},
    {"algorithm": "CLHPSO", "HCI": 1, "CL": 1, "SHDL": 0, "LPSR": 0,
     "description": "HPSO with comprehensive learning."},
    {"algorithm": "SHCLPSO", "HCI": 1, "CL": 1, "SHDL": 1, "LPSR": 0,
     "description": "CLHPSO with success-history differential learning."},
    {"algorithm": "SCLPSO", "HCI": 1, "CL": 1, "SHDL": 1, "LPSR": 1,
     "description": "Final Success-history Comprehensive Learning PSO with linear population-size reduction."},
]


def save_clpso_version_definition_table(out_root: str) -> str:
    df = pd.DataFrame(SCLPSO_VERSION_DEFINITIONS)
    path = os.path.join(out_root, "Table_5_style_SCLPSO_variant_definitions.csv")
    df.to_csv(path, index=False)
    return path


def _rank_table_markdown(df: pd.DataFrame, cols: List[str], max_rows: int = 20) -> str:
    if df is None or df.empty:
        return "No available data.\n"
    sub = df[cols].copy().head(max_rows)
    return sub.to_markdown(index=False)


def save_section5_style_report(raw_df: pd.DataFrame,
                               summary_df: pd.DataFrame,
                               rank_df: pd.DataFrame,
                               wtl_df: pd.DataFrame,
                               out_root: str) -> str:

    version_df = pd.DataFrame(SCLPSO_VERSION_DEFINITIONS)

    if rank_df is not None and not rank_df.empty:
        overall = rank_df.groupby("algorithm", as_index=False)["rank"].mean().sort_values("rank")
        overall.rename(columns={"rank": "average_rank"}, inplace=True)
    else:
        overall = pd.DataFrame(columns=["algorithm", "average_rank"])


    dim_rank = rank_df.sort_values(["dimension", "rank"]) if rank_df is not None and not rank_df.empty else pd.DataFrame()


    cat_path = os.path.join(out_root, "category_average_rankings.csv")
    cat = pd.read_csv(cat_path) if os.path.exists(cat_path) else pd.DataFrame()

    best_algo = overall.iloc[0]["algorithm"] if not overall.empty else "SCLPSO"
    target_row = overall[overall["algorithm"] == "SCLPSO"] if not overall.empty else pd.DataFrame()
    target_rank = float(target_row.iloc[0]["average_rank"]) if not target_row.empty else float("nan")

    lines = []
    lines.append("# Section 5-style variant analysis of SCLPSO on CEC2017\n")
    lines.append("## Proposed algorithm name\n")
    lines.append("The final optimizer is named **SCLPSO**, standing for **Success-history Comprehensive Learning Particle Swarm Optimizer**. ")
    lines.append("The name highlights the two main search mechanisms retained in the final algorithm: comprehensive learning in the PSO backbone and success-history-guided differential learning. Halton-chaotic initialization and linear population-size reduction are used as auxiliary mechanisms. The previously tested state-guidance, DVA, and escape operator are not included in the final version because preliminary experiments showed unstable or negative contribution.\n")
    lines.append("## 5.1 Mechanism construction and variant settings\n")
    lines.append("Following the style of the reference paper, the proposed algorithm is decomposed into a set of mechanism-controlled variants. Each variant switches on or off a specific mechanism group, so the contribution of each component can be observed directly. The experimental setting follows CEC2017 with 51 independent runs, dimensions 10D, 30D, 50D, and 100D, all functions F1-F30, and maxFEs = 10000 x D.\n")
    lines.append(_rank_table_markdown(version_df, ["algorithm", "HCI", "CL", "SHDL", "LPSR", "description"], max_rows=20))
    lines.append("\n## 5.2 Experimental comparison of SCLPSO variants\n")
    lines.append("The average ranking results are used as the primary comparison criterion, where a smaller rank indicates better overall performance. The main purpose is to show the stepwise effect of each added mechanism from the basic PSO to the final SCLPSO, following the same progressive-variant logic used by the reference paper.\n")
    if not overall.empty:
        lines.append("### Overall average ranking\n")
        lines.append(_rank_table_markdown(overall, ["algorithm", "average_rank"], max_rows=30))
        lines.append("\n")
        lines.append(f"The best overall variant is **{best_algo}**. The final SCLPSO obtains an overall average rank of **{target_rank:.3f}**. If a predecessor ranks above SCLPSO, the final mechanism should be reconsidered rather than forcing an over-parameterized algorithm.\n")
    if not dim_rank.empty:
        lines.append("### Dimension-wise average ranking\n")
        lines.append(_rank_table_markdown(dim_rank, ["dimension", "algorithm", "rank"], max_rows=80))
        lines.append("\n")
    if not cat.empty:
        lines.append("### Function-category average ranking\n")
        lines.append(_rank_table_markdown(cat.sort_values(["category", "rank"]), ["category", "algorithm", "rank"], max_rows=80))
        lines.append("\n")
    if wtl_df is not None and not wtl_df.empty:
        lines.append("### Win/tie/loss against final SCLPSO\n")
        lines.append(_rank_table_markdown(wtl_df, ["dimension", "competitor", "SCLPSO_wins", "ties", "SCLPSO_losses"], max_rows=120))
        lines.append("\n")
    lines.append("## Figures generated by the script\n")
    lines.append("- Figure 5-style convergence curves: `SCLPSO_Convergence_Curves.jpg` and `convergence_curves_by_function/`.\n")
    lines.append("- Figure 6-style boxplots: `boxplots_by_function/`.\n")
    lines.append("- Average-rank summaries: `SCLPSO_Friedman_Average_Rank_Bar.jpg`, `SCLPSO_Dimension_Average_Rankings.jpg`, `SCLPSO_Function_Category_Rankings.jpg`, and `SCLPSO_Average_Rank_Radar.jpg`.\n")
    lines.append("- Statistical comparison: `SCLPSO_Wilcoxon_Rank_Sum_Heatmap.jpg` and `SCLPSO_Win_Tie_Loss.jpg`.\n")
    lines.append("\n## Suggested manuscript wording\n")
    lines.append("To avoid presenting the method as a simple strategy stack, the manuscript should state that SCLPSO is designed as a progressive PSO framework. Each mechanism is added in sequence and evaluated as a named variant, following the same logic as the reference paper's HGS/LHGS/SCHGS/LSCHGS comparison. The final configuration should be supported by the overall average ranking and the win/tie/loss results.\n")
    path = os.path.join(out_root, "paper_section_5_1_5_2_style_SCLPSO_report.md")
...
def save_reports(raw_rows: List[Dict[str, Any]], curve_rows: List[Dict[str, Any]], out_root: str, final: bool = False):
    ...
    if not raw_df.empty:
        ...
        wtl_df = win_tie_loss_against_acpso(summary_df, target="SCLPSO")
        ...
        if final:
            try:
                version_path = save_clpso_version_definition_table(out_root)
                print(f"SCLPSO version-definition table: {version_path}", flush=True)
            except Exception as e:
                print(f"[WARNING] Failed to save SCLPSO version table: {e}", flush=True)
            plot_paths = generate_visual_outputs(raw_df, summary_df, rank_df, wtl_df, curve_rows, out_root, final=True)
            try:
                report_path = save_section5_style_report(raw_df, summary_df, rank_df, wtl_df, out_root)
                print(f"Section-5-style report: {report_path}", flush=True)
            except Exception as e:
                print(f"[WARNING] Failed to save section-5-style report: {e}", flush=True)
            if plot_paths:
...
def main():
    ...
    print("=" * 100)
    print("SCLPSO Section-5-style variant experiment on CEC2017 F1-F30 including F2")
    ...
    print(f"Functions:      {format_function_list(funcs) if not CFG.TEST_MODE else [1, 2, 3]}")
    print(f"Exclude F2:     {CFG.EXCLUDE_F2}")
    if CFG.EXCLUDE_F2 and not CFG.TEST_MODE:
        print("F2 note:        legacy mode is enabled; F2 is excluded.")
    elif not CFG.TEST_MODE:
        print("F2 note:        F2 is included; the experiment uses all 30 functions F1-F30.")
    print(f"Runs:           {CFG.N_RUNS if not CFG.TEST_MODE else 2}")
    print(f"MaxFEs:         {CFG.MAX_FES_FACTOR} * D")
    print(f"Population:     {CFG.POP_SIZE}")
    print(f"Algorithms:     {CFG.ALGORITHMS if not CFG.TEST_MODE else ('PSO', 'SCLPSO')}")
    ...
    print(f"Variant contribution:    {os.path.join(CFG.OUT_ROOT, 'progressive_variant_contribution_summary.csv')}")
    ...
    print(f"Variant contrib. fig.:   {os.path.join(CFG.OUT_ROOT, 'SCLPSO_Progressive_Variant_Contribution_Bar.jpg')}")
